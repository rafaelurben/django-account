        ...
        if hasattr(backend, 'get_user'):
            try:
                if backend.get_user(user.pk) == user:
                    ...
                    return
            except (user.__class__.DoesNotExist, AttributeError, TypeError, ValueError, ImproperlyConfigured):
                logger.warning("Backend %s failed to authenticate user %s: %s", backend, user, exc_info=True)
                continue
